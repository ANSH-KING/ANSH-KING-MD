...
const cap = `*ANSH-KING-MD VIDEO DOWNLOADER 🛜*
*TITLE: ${anu.title}*
...
*ᴀɴꜱʜ-ᴋɪɴɢ-ᴍᴅ ʙᴏᴛ ᴄʀᴇᴀᴛᴇᴅ ʙʏ ᴀɴꜱʜʟᴏᴠᴇʀ⁷⁰²*

*ᴊᴏɪɴ ғᴏʀ ᴀɴꜱʜ-ᴋɪɴɢ-ᴍᴅ ᴜᴘᴅᴀᴛᴇs*
...
let senda = await conn.sendMessage(from, { video: {url: await yt.video['360p'].download() }, caption: '*VIDEO BY ANSH-KING-MD*'}, { quoted: mek })  
...
let senda1 = await conn.sendMessage(from, { video: {url: await yt.video['720p'].download() }, caption: '> ᴠɪᴅᴇᴏ ʙʏ ᴀɴꜱʜ-ᴋɪɴɢ-ᴍᴅ ʙᴏᴛ ✅'}, { quoted: mek })  
...
let search = await yts(q)
let anu = search.videos[0]
const cap = `*ANSH-KING-MD MUSIC DOWNLOADER 🛜*
...
var du = await yt2.audio['128kbps'].download()
    let senda =  await conn.sendMessage(from, { document: { url : du }, mimetype: 'audio/mpeg', fileName: yt2.title + '.mp3',caption: '> ᴍᴜsɪᴄ ʙʏ ᴀɴꜱʜ-ᴋɪɴɢ-ᴍᴅ ʙᴏᴛ ✅' }, { quoted: mek })
    await conn.sendMessage(from, { react: { text: '🎼', key: senda.key }})
